...
---
### 2025.11.12 - 크론잡 썸네일 자동 생성 추가

#### 주요 변경사항

**fix(cron): 블로그 생성 후 썸네일 자동 생성**
- `setup_cron.sh` 업데이트
  - 정책 수집 → 블로그 생성 → **썸네일 생성** 순차 실행
  - 각 단계별 성공/실패 로깅
  - 이전 단계 실패 시 다음 단계 건너뛰기

**feat(thumbnail): 썸네일 일괄 생성 스크립트**
- `jobs/thumbnail_refresh.py`
  - DB에서 `thumbnail_url`이 비어있는 블로그 조회
  - 정책 정보 기반 LLM 캡션 생성
  - 카테고리별 배경 이미지 자동 선택
  - PIL로 이미지 생성 및 S3 업로드
  - DB 자동 업데이트
...
[블로그 생성] generate_all_blogs.py
    ↓
[썸네일 생성] thumbnail_refresh.py ← 새로 추가
    ↓
완료
```
...
- 백엔드 `prompts.py`
  - `type=meta` 추가: 별도 LLM 호출 없이 기존 값으로 메타 구성
    ...
    - `thumbnail_img`: S3 URL(옵션, 향후 확장 예정)
    - `robots`: 검색 로봇 제어 값
  - `type=full` 보완: 본문(`blog_content`)과 함께 위 메타 구조 포함해 반환
  - `type=blog`: 생성된 본문에 `add_blog_footer` 적용 (안내 문구 + 참고 링크 자동 부착)
...
- 프론트엔드
  ...
  - `PostDetailPage.tsx`: `<Helmet>` 블록으로 SEO 메타 태그 주입 + 레이아웃 개선

### 동작 요약
- 메타 태그 => 백엔드가 조립한 값을 프론트가 `<Helmet>`으로 `<head>`에 주입.
- `thumbnail_img`는 공유 시 뜨는 미리보기 이미지로, 현재는 로고/썸네일 확장 시 사용 예정.
